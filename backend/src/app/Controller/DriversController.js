...
class DriverController {
        ...
        try{
            ...
            .query(sqlString, (err, data) => {
                if(!err){
                    ...
                    res.json({message: "Không thể tìm kiếm được", error: err.message});
                }
            })
        }catch(err){
            console.log(err);
    ...
    createDriver = async (req, res) => {
        try{
            ...
            .input('sdtTX', sql.Int, req.body.sdtTX)
            .input('dia_chi_TX', sql.NVarChar, req.body.dia_chi_TX)
            .query(sqlString, (err, data) => {
                if(!err) {
                    res.json({message: "Thêm thông tin thành công", result: data.recordset});
                }else{
